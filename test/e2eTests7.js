...
describe("End-to-End Tests - Seven", function() {
  ...
  it("Add fully realistic test (actual variables we'll use and two treasuries and mining and a dispute)", async function() {
    ...
    await oracle.connect(accounts[1]).submitValue(h.tob32("1"),300,nonce);
    await h.advanceTime(300);
    nonce = await oracle.getTimestampCountById(h.tob32("2"));
    await oracle.connect(accounts[2]).submitValue(h.tob32("2"),500,nonce);
    ...
    await h.advanceTime(86400);
    nonce = await oracle.getTimestampCountById(h.tob32("1"));
    await oracle.connect(accounts[1]).submitValue(h.tob32("1"),1000,nonce);
    ...
    await governance.connect(accounts[3]).beginDispute(h.tob32("1"),blocky.timestamp);
    voteCount = await governance.voteCount();
    await govTeam.vote(voteCount,true,false);
    await govBig.vote(voteCount,true,false);
    ...
    await h.advanceTime(604800);
    nonce = await oracle.getTimestampCountById(h.tob32("1"));
    h.expectThrow(oracle.connect(accounts[1]).submitValue(h.tob32("1"),300,nonce));//disputed miner can't submit vals
    nonce = await oracle.getTimestampCountById(h.tob32("2"));
    await oracle.connect(accounts[2]).submitValue(h.tob32("2"),500,nonce);
    ...
    nonce = await oracle.getTimestampCountById(h.tob32("1"));
    h.expectThrow(oracle.connect(accounts[1]).submitValue(h.tob32("1"),300,nonce));//kicked miner can't submit vals
    nonce = await oracle.getTimestampCountById(h.tob32("2"));
    await oracle.connect(accounts[2]).submitValue(h.tob32("2"),500,nonce);
  });
